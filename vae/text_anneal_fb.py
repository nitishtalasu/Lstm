...
def main(args):
    ...
    if args.reconstruct_from != "":
        ...
        vae.eval()
        with torch.no_grad():
            test_data_batch, test_data_labels = test_data.create_data_batch_labels(batch_size=1,
                                                          device=device,
                                                          batch_first=True)
            # test(vae, test_data_batch, "TEST", args)
            reconstruct(vae, test_data_batch, test_data_labels, vocab, args.decoding_strategy, args.reconstruct_to)
    ...
    try:
        for epoch in range(args.epochs):
            ...
            if args.save_ckpt > 0 and epoch <= args.save_ckpt:
                ...
                s = 'model_ckpt_' + str(epoch) + '.pt'
                torch.save(vae.state_dict(), os.path.join(args.exp_dir, s))
