...
class VAE(nn.Module):
    ...
    def GenerateDocuments(self, x):
        
        latent_z = list()
        for batch_data in x:
            z = self.sample_from_inference(batch_data).squeeze(1)
            for item in z:
                latent_z.append(item)

        #z = self.sample_from_inference(x).squeeze(1)
        z = latent_z
        num_of_docs = len(z)
        generated_documents = list()
        for i in range(0, num_of_docs//2):
            for j in range(num_of_docs // 2, num_of_docs):
                interpolations = self.interpolate(z[i].cpu().detach().numpy(), z[j].cpu().detach().numpy())
                interpolations = torch.from_numpy(interpolations)#.to(self.device)
                generated_documents.extend(self.decode(interpolations, "sample", K=5))
                # for i in interpolations:
                #     result = self.decode(z, "beam", K=5)
                #     #result = self.beam_decode(z=i)
                #     generated_documents.append(result)
        
        #generated_documents = self.decode(z, "beam", K=5)
        return generated_documents

    
    def interpolate(self, start, end, steps=4):
        interpolation = np.linspace(start, end, num=10, endpoint=True)
