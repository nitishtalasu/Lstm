...
def reconstruct(model, test_data_batch, test_data_labels, vocab, strategy, fname):
    hyps = []
    refs = []
    label_index = defaultdict(list)
    for idx, label in enumerate(test_data_labels):
        label_index[label[0]].append(idx)
    # print(label_index)

    with open(fname, "w") as fout:
      with open("pseudo_documents.txt", "w") as f:
        for label, idx in label_index.items():
          classified_documents = []
          for i in idx:
            classified_documents.append(test_data_batch[i])
          decoded_batch_interpolation = model.GenerateDocuments(classified_documents)
        
          for doc in decoded_batch_interpolation:
              a = " ".join(doc)
              f.write(label + "\t" + a + "\n")
