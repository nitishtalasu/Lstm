...
def pseudodocs_vae(word_sup_array, total_num, background_array, sequence_length, len_avg,
                len_std, num_doc, interp_weight, vocabulary, vocabulary_inv, embedding_mat, model, save_dir=None):
        
    with open("pseudo_documents.txt", "r") as f:
        lines = f.readlines()
        docs = list()
        label = list()
        for line in lines:
        ...
        docs = preprocess_doc(docs)
        docs = [s.split(" ") for s in docs]
        docs = pad_sequences(docs, pad_len=sequence_length)
        pad_token = vocabulary["<PAD/>"]
        docs = [[vocabulary.get(word, pad_token) for word in sentence] for sentence in docs]
        np_docs = np.zeros((len(docs), sequence_length), dtype='int32')
        label = np.array(label)
        encoded_label = to_categorical(label)
        background_class_prob = interp_weight/len(encoded_label[0])
        label_class_prob = (1 - interp_weight) + background_class_prob
        for i, doc in enumerate(docs):
            for j, word in enumerate(doc):
                if j >= sequence_length:
                    break 
                np_docs[i][j] = word
            for category, label in enumerate(encoded_label[i]):
                if label == 1:
                    encoded_label[i][category] = label_class_prob
                else:
                    encoded_label[i][category] = background_class_prob

    print("Finished Pseudo documents generation.")
    return np_docs, encoded_label
